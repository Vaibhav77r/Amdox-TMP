  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            {STATUSES.map(s => (
              <button key={s}
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all
                  ${filterStatus === s ? 'bg-primary-600 text-white' : 'bg-dark-800 text-dark-400 hover:text-dark-200'}`}>
          ...
          {(user?.role === 'ADMIN' || user?.role === 'EDITOR') && (
            <button onClick={() => setCreateOpen(true)} className="btn-primary ml-auto flex items-center gap-2">
          ...
          {PRIORITIES.map(p => (
            <button key={p}
              ...
              className={`px-3 py-1 rounded-lg text-xs font-medium transition-all
                ${filterPriority === p ? 'bg-dark-700 text-dark-100 border border-dark-500' : 'text-dark-500 hover:text-dark-300'}`}>
                ...
                <thead>
                  <tr className="border-b border-dark-800">
                    {['Task', 'Status', 'Priority', 'Assignee', 'Due Date', 'Actions'].map(h => (
                      <th key={h} className="text-left px-5 py-3.5 text-xs font-semibold text-dark-500 uppercase tracking-wider">
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center py-12 text-dark-500">
                        No tasks found. {user?.role !== 'VIEWER' && 'Create your first task!'}
                      </td>
                    </tr>
                  ) : filtered.map(task => (
                    <tr key={task.id} className="hover:bg-dark-800/40 transition-colors group">
                      ...
                      <td className="px-5 py-4">
                        <span className={`flex items-center gap-1.5 text-xs
                          ${isOverdue(task.dueDate) && task.status !== 'DONE' ? 'text-red-400' : 'text-dark-400'}`}>
                          <Calendar size={11} />
      ...
      <TaskDetailDrawer
        ...
        onEdit={() => { setEditTask(tasks.find(t => t.id === detailId)); setDetailId(null) }}
      />
      <ConfirmDialog
        isOpen={!!deleteTask}
